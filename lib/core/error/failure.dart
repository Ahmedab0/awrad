...
abstract class Failure {
  ...
  //Failure({required this.apiErrorModel});
}
      ...
      case DioExceptionType.unknown:
        if (dioExc.message!.contains('SocketException')) {
          return ServerFailure(
              errorMsg: 'No Internet Connection: SocketException');
        } else {
          return ServerFailure(errorMsg: 'Unexpected Error, Please try again!');
        }
      default:
        return ServerFailure(
            errorMsg: 'Opps There was an Error, Please try again');
    }
  }

  factory ServerFailure.fromResponse(int? statusCode, dynamic response) {
    if (statusCode == 400 || statusCode == 401 || statusCode == 403) {
      log('response ==>> ${response["msg"]}');
